...
def scatter_grid_pool(points, lengths, grid_size, feat=None, start=None):
    ...
    # Get offset and batch indices
    offset = torch.cumsum(lengths, dim=0)
    offset0 = torch.cat([torch.zeros(1, dtype=offset.dtype, device=offset.device), offset], dim=0)
    batch = offset2batch(offset)

    start = segment_csr(points, offset0, reduce="min") if start is None else start
    
    vox_cluster = voxel_grid(pos=points - start[batch], size=grid_size, batch=batch, start=0)
    unique, cluster, counts = torch.unique(vox_cluster, sorted=True, return_inverse=True, return_counts=True)

    _, sorted_cluster_indices = torch.sort(cluster)
    idx_ptr = torch.cat([counts.new_zeros(1), torch.cumsum(counts, dim=0)])
    sub_points = segment_csr(points[sorted_cluster_indices], idx_ptr, reduce="mean")
    batch = batch[idx_ptr[:-1]]
    offset = batch2offset(batch)

    offset = torch.cat([torch.zeros(1, dtype=offset.dtype, device=offset.device), offset], dim=0)
    sub_lengths = offset[1:] - offset[:-1]

    # Optionnaly apply maxpool to features
    if feat is not None:
        feat = segment_csr(feat[sorted_cluster_indices], idx_ptr, reduce="max")

    return sub_points, sub_lengths, sorted_cluster_indices, idx_ptr, cluster
    ...
    for i in range(num_layers):
        ...
        if i > 0:
            ...
            if grid_pool_mode:
                ...
                sub_points, sub_lengths, pool_sorting, pool_ptr, upsamplings = scatter_grid_pool(points0, lengths0, sub_size)

                pyramid.pools.append((pool_sorting, pool_ptr))
                pyramid.upsamples.append(upsamplings.reshape(-1, 1))
